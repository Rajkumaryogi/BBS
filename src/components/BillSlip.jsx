...
export default function BillSlip({ cart, gstEnabled, gstRate, billNo, tableNo, coverCount, waiterNo }) {
  ...
  return (
    <div className="bill-slip">
      <div className="slip-header">
        ...
        {gstEnabled && <div className="slip-contact" style={{fontSize:"10px"}}>GSTIN: 27AAAAA0000A1Z5</div>}
      </div>
      ...
      <div className="slip-item-row subtot-row">
        ...
        <span className="sc-qty"></span><span className="sc-rate"></span>
        <span className="sc-amt">{fmt(subtotal)}</span>
      </div>
      {gstEnabled && <>
        <div className="slip-item-row">
          <span className="sc-desc">CGST @{halfRate}%</span>
          <span className="sc-qty"></span><span className="sc-rate"></span>
          <span className="sc-amt">{fmt(cgst)}</span>
        </div>
        <div className="slip-item-row">
          <span className="sc-desc">SGST @{halfRate}%</span>
          <span className="sc-qty"></span><span className="sc-rate"></span>
          <span className="sc-amt">{fmt(sgst)}</span>
        </div>
      </>}
      <div className="slip-dots">{"." .repeat(40)}</div>
      ...
      <div className="slip-footer-row">
        <span>C 6</span><span>{time}</span><span>M/C NO</span><span>1</span>
      </div>
    </div>
  );
}
